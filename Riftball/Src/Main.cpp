...
const int NumberOfSliders = 10; GameObject planeForSliders[NumberOfSliders]; GameObject ButtonForSliders[NumberOfSliders]; Sliders Slider[NumberOfSliders];//Sliders
const int NumberOfButtons = 7; GameObject ButtonObjects[NumberOfButtons]; Buttons Button[NumberOfButtons];//Buttons
const int NumberOfBorders = 1; GameObject Borders[NumberOfBorders];//Boarders
GameObject NumbersPlanes[4];
...
void InMenuDraw(int Inum)
{
	...
	passThroughMaterial->mat4Uniforms["mvm"] = modelViewMatrix[Inum];
	passThroughMaterial->mat4Uniforms["prm"] = projectionMatrix[Inum];
	passThroughMaterial->mat4Uniforms["u_mvp"] = (modelViewMatrix[Inum] * projectionMatrix[Inum]);
	passThroughMaterial->mat4Uniforms["u_mv"] = modelViewMatrix[Inum];
	passThroughMaterial->mat4Uniforms["u_lightPos_01"] = (modelViewMatrix[Inum] * glm::translate(glm::mat4(1.0f), lightPosition_01));
	passThroughMaterial->mat4Uniforms["u_lightPos_02"] = (modelViewMatrix[Inum] * glm::translate(glm::mat4(1.0f), lightPosition_02));
	passThroughMaterial->sendUniforms();
...
void InGameDraw(int Inum)
{
	...
	passThroughMaterial->mat4Uniforms["mvm"] = modelViewMatrix[PlayerTeam[Inum]];
	passThroughMaterial->mat4Uniforms["prm"] = projectionMatrix[PlayerTeam[Inum]];
	passThroughMaterial->mat4Uniforms["u_mvp"] = (modelViewMatrix[PlayerTeam[Inum]] * projectionMatrix[PlayerTeam[Inum]]);
	passThroughMaterial->mat4Uniforms["u_mv"] = modelViewMatrix[PlayerTeam[Inum]];
	passThroughMaterial->mat4Uniforms["u_lightPos_01"] = (modelViewMatrix[PlayerTeam[Inum]] * glm::translate(glm::mat4(1.0f), lightPosition_01));
	passThroughMaterial->mat4Uniforms["u_lightPos_02"] = (modelViewMatrix[PlayerTeam[Inum]] * glm::translate(glm::mat4(1.0f), lightPosition_02));
	passThroughMaterial->sendUniforms();

	NASDMaterial->mat4Uniforms["mvm"] = modelViewMatrix[PlayerTeam[Inum]];
	NASDMaterial->mat4Uniforms["prm"] = projectionMatrix[PlayerTeam[Inum]];
	NASDMaterial->mat4Uniforms["u_mvp"] = (modelViewMatrix[PlayerTeam[Inum]] * projectionMatrix[PlayerTeam[Inum]]);
	NASDMaterial->mat4Uniforms["u_mv"] = modelViewMatrix[PlayerTeam[Inum]];
	NASDMaterial->mat4Uniforms["u_lightPos_01"] = (modelViewMatrix[PlayerTeam[Inum]] * glm::translate(glm::mat4(1.0f), lightPosition_01));
	NASDMaterial->mat4Uniforms["u_lightPos_02"] = (modelViewMatrix[PlayerTeam[Inum]] * glm::translate(glm::mat4(1.0f), lightPosition_02));
	NASDMaterial->sendUniforms();
	...
	NumbersPlanes[1].setTexture(TextureNumbers[static_cast<int>(Health[0] % 10)]);
	NumbersPlanes[0].setTexture(TextureNumbers[static_cast<int>(Health[0] / 10 % 10)]);

	NumbersPlanes[3].setTexture(TextureNumbers[static_cast<int>(Health[1] % 10)]);
	NumbersPlanes[2].setTexture(TextureNumbers[static_cast<int>(Health[1] / 10 % 10)]);


	for (unsigned int i = 0; i < 4; i++) {
		if (NumbersPlanes[i].Viewable) {
			if (NumbersPlanes[i].textureHandle_hasTransparency == true) { disableCulling(); }
			else { enableCulling(); }

			NumbersPlanes[0].setRotation(glm::vec3(0.0f, 90.0f, 270.0f));
			NumbersPlanes[0].setPosition(glm::vec3(-46.00f, 7.0f, 32.50f));
			NumbersPlanes[1].setRotation(glm::vec3(0.0f, 90.0f, 270.0f));
			NumbersPlanes[1].setPosition(glm::vec3(-46.00f, 7.0f, 28.0f));

			NumbersPlanes[2].setRotation(glm::vec3(0.0f, 90.0f, 270.0f));
			NumbersPlanes[2].setPosition(glm::vec3(-46.00f, 7.0f, -28.0f));
			NumbersPlanes[3].setRotation(glm::vec3(0.0f, 90.0f, 270.0f));
			NumbersPlanes[3].setPosition(glm::vec3(-46.00f, 7.0f, -32.50f));
			NumbersPlanes[i].drawObject();


			NumbersPlanes[0].setRotation(glm::vec3(180.0f, 270.0f, 270.0f));
			NumbersPlanes[0].setPosition(glm::vec3(46.00f, 7.0f, -32.50f));
			NumbersPlanes[1].setRotation(glm::vec3(180.0f, 270.0f, 270.0f));
			NumbersPlanes[1].setPosition(glm::vec3(46.00f, 7.0f, -28.0f));

			NumbersPlanes[2].setRotation(glm::vec3(180.0f, 270.0f, 270.0f));
			NumbersPlanes[2].setPosition(glm::vec3(46.00f, 7.0f, 28.0f));
			NumbersPlanes[3].setRotation(glm::vec3(180.0f, 270.0f, 270.0f));
			NumbersPlanes[3].setPosition(glm::vec3(46.00f, 7.0f, 32.50f));
			NumbersPlanes[i].drawObject();
		}
	}

	//Objects[7].drawTileSet(1.0f, 1.0f, 0.1f, 0.1f);
	//Objects[8].drawTileSet(130.0f, 100.0f, 130.0f, 100.0f);
	...
	if (CollisionBetweenObjects) {
		...
		for (int i = 0; i < NumberOfPlayers; i++) {
			...
			for (int j = 0; j < NumberOfObjects; j++) {
				if (Objects[j].Viewable && (j != 7 && j != 8 && j != 9)) {
					m.A = Objects[j];
					//avoid objects
...
void KeyboardCallbackFunction(unsigned char key, int x, int y)
{
	...
	if (inMenu) {
		...
		case 'v':
			...
			break;
		case '0':
			Specials[static_cast<int>(TestFloat)].Viewable = true;
			Specials[static_cast<int>(TestFloat)].setPosition(glm::vec3(0.0f, 20.0f, 0.0f));
			Specials[static_cast<int>(TestFloat)].setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
			break;
		case '1':
			FxChannel[0] = Fx[0].Play();
...
void InitializeNumbers() {
	...
	NumbersPlanes[0].objectLoader(ObjectPath + "PlainForText.obj");
	NumbersPlanes[0].setMaterial(passThroughMaterial);
	NumbersPlanes[0].setMass(0.0f);
	NumbersPlanes[0].setScale(glm::vec3(2.50f, 2.0f, 5.0f));
	NumbersPlanes[0].setRotation(glm::vec3(180.0f, 90.0f, 270.0f));
	NumbersPlanes[0].setPosition(glm::vec3(46.00f, 5.0f, 32.50f));
	NumbersPlanes[0].setTexture(ilutGLLoadImage(_strdup((ImagePath + "HUD//Numbers//Numbers_00.png").c_str())));
	NumbersPlanes[1].objectLoader(&NumbersPlanes[0]);
	NumbersPlanes[1].setRotation(glm::vec3(180.0f, 90.0f, 270.0f));
	NumbersPlanes[1].setPosition(glm::vec3(46.00f, 5.0f, 28.0f));

	NumbersPlanes[2].objectLoader(&NumbersPlanes[0]);
	NumbersPlanes[2].setRotation(glm::vec3(0.0f, 270.0f, 270.0f));
	NumbersPlanes[2].setPosition(glm::vec3(46.00f, 5.0f, 32.50f));
	NumbersPlanes[3].objectLoader(&NumbersPlanes[0]);
	NumbersPlanes[3].setRotation(glm::vec3(0.0f, 270.0f, 270.0f));
	NumbersPlanes[3].setPosition(glm::vec3(46.00f, 5.0f, 28.0f));

	//NumbersPlanes[2].objectLoader(&NumbersPlanes[0]);
	//NumbersPlanes[2].setRotation(glm::vec3(0.0f, 270.0f, 270.0f));
	//NumbersPlanes[2].setPosition(glm::vec3(-46.00f, 5.0f, 30.0f));
	//NumbersPlanes[3].objectLoader(&NumbersPlanes[0]);
	//NumbersPlanes[3].setRotation(glm::vec3(0.0f, 270.0f, 270.0f));
	//NumbersPlanes[3].setPosition(glm::vec3(-46.00f, 5.0f, -30.0f));
...
void InitializeShaders()
{
	...
	//Load vertex shaders
	Shader v_default, v_passThrough, v_textShader, v_NASD;
	...
	v_NASD.loadShaderFromFile(shaderPath + "NASD_v.glsl", GL_VERTEX_SHADER);
	...
	materials["NASD"]->shader->attachShader(v_NASD);
...
void InitializeObjects()
{
	...
	Borders[0].setMaterial(NASDMaterial);
	...
	Players[0].setMaterial(NASDMaterial);
	...
	for (int i = 0; i < NumberOfPlayers; i++) {
		if (PlayerTeam[i] == 0) {
			...
			AffectsOnPlayer[i][0].setTexture(ilutGLLoadImage(_strdup((ImagePath + "Player//ShockWave_01.png").c_str())));
		}
		else if (PlayerTeam[i] == 1) { 
			Players[i].setTexture(ilutGLLoadImage(_strdup((ImagePath + "Player//R_blitzbot_diff.png").c_str())));
			AffectsOnPlayer[i][0].setTexture(ilutGLLoadImage(_strdup((ImagePath + "Player//ShockWave_02.png").c_str())));
		}
	}
	...
	Rifts[0].setMaterial(NASDMaterial);
	Rifts[0].setMass(0.0f);
	Rifts[0].setScale(glm::vec3(2.0f, 16.0f, 30.0f));
	Rifts[0].setSizeOfHitBox(glm::vec3(2.0f, 24.0f, 30.0f));
	Rifts[0].setPosition(glm::vec3(48.0f, 8.0f, 0.0f));
	...
	Rifts[1].setPosition(glm::vec3(-48.0f, 8.0f, 0.0f));
	...
	Specials[0].setMaterial(NASDMaterial);
	...
	Specials[1].setMaterial(NASDMaterial);
	...
	Specials[2].setMaterial(NASDMaterial);
	...
	Specials[3].setMaterial(NASDMaterial);
	...
	Specials[4].setMaterial(NASDMaterial);
	...
	Specials[5].setMaterial(NASDMaterial);
	...
	Specials[6].setMaterial(NASDMaterial);
	...
	Specials[7].setMaterial(NASDMaterial);
	...
	Specials[8].setMaterial(NASDMaterial);
	...
	Specials[9].setMaterial(NASDMaterial);
	...
	Objects[0].setMaterial(NASDMaterial);
	...
	Objects[0].setTexture(ilutGLLoadImage(_strdup((ImagePath + "Rifts//Ground_03.png").c_str())));
	Objects[0].setTexture(1, ilutGLLoadImage(_strdup((ImagePath + "NormalMap.png").c_str())));

	//Walls Left
	Objects[1].objectLoader(ObjectPath + "Rift//stadium.obj");
	Objects[1].setMaterial(NASDMaterial);
	...
	Objects[2].setMaterial(NASDMaterial);
	Objects[2].setPosition(glm::vec3(53.0f, 08.0f, -20.0f));
	...
	Objects[3].setMaterial(NASDMaterial);
	Objects[3].setPosition(glm::vec3(53.0f, 08.0f, 20.0f));
	...
	Objects[4].setPosition(glm::vec3(-53.0f, 08.0f, 20.0f));
	...
	Objects[5].setPosition(glm::vec3(-53.0f, 08.0f, -20.0f));
	...
	Objects[6].setMaterial(NASDMaterial);
	...
	Objects[7].objectLoader(ObjectPath + "Rift//baners_01.obj");
	Objects[7].setMaterial(NASDMaterial);
	Objects[7].setTexture(ilutGLLoadImage(_strdup((ImagePath + "HUD//HUD_Display_B.png").c_str())));
	Objects[8].objectLoader(ObjectPath + "Rift//baners_02.obj");
	Objects[8].setMaterial(NASDMaterial);
	Objects[8].setTexture(ilutGLLoadImage(_strdup((ImagePath + "HUD//HUD_Display_R.png").c_str())));

	//GLuint textureTemp = ilutGLLoadImage(_strdup((ImagePath + "HUD//Font.png").c_str()));
	//glTexStorage3D(GL_TEXTURE_2D_ARRAY, GL_MAX_TEXTURE_SIZE, GL_RGB, 130, 100, 100); //x,y,amount
	//float mip_level = 0, xoffset = 0, yoffset = 0, zoffset = 0;
	//glTexSubImage3D(GL_TEXTURE_2D_ARRAY, mip_level, xoffset, yoffset, zoffset, 130, 100, 1, GL_RGB, GL_UNSIGNED_BYTE, incoming_data);
	...
	Enemies[0].setMaterial(NASDMaterial);
	...
	ShadowObject[0].setMaterial(NASDMaterial);
...
void init()
{
	...
	for (int i = 0; i < NumberOfPlayers; i++) {
		...
		for (int j = 0; j < NumberOfSpecials; j++) {
			...
			AbilityCounter[i][j] = 4.0f;
			AbilityLength[i][j] = 4.0f;
		}
		AbilityLength[i][6] = 1.0f;
		AbilityCounter[i][6] = 1.0f;
	}
